...
class Experiment:
    ...
    def normalize_images(self):
        self.images_flattened = self.images_flattened / 255.0

    def standardize_images(self):
        # Calculate the mean and standard deviation of the pixel values
        mean = np.mean(self.images_flattened, axis=0)
        std_dev = np.std(self.images_flattened, axis=0) + 1e-20
        # Normalize the images by subtracting the mean and dividing by the standard deviation
        standardized_images = (self.images_flattened - mean) / std_dev
        self.images_flattened = standardized_images
    ...
    def run(self): 
        with open(f'{self.path}/{self.distance}{"-reduced-"+str(self.reduce_with_pca)+"-" if self.reduce_with_pca else ""}{ "normalized-" if self.normalize else ""}{time.time()}.txt', 'w') as out_file:
            ...
            print("Clustering accuracy using Euclidean distance:", self.calculate_clustering_accuracy())
